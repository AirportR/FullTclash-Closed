...
- 去除了订阅转换中默认启动emoji的设置
- 新增**中文**维基百科检测，此测试项区别于维基百科（英文）。
- 配置文件改动：改动了颜色相关配置，请前往 config.yaml.example 查看，旧版配色配置已失效。
- 配置新增：自定义绘图标题、自定义**解锁**颜色、自定义**失败**颜色、自定义落地ip风险颜色、自定义**待解**颜色、自定义**N/A**颜色。
请前往 config.yaml.example 查看。
- 订阅管理优化：订阅的所有者可以使用 /share 指令给予其他人测试某个订阅的权限
- 新增 /invite 指令。用户权限可以邀请游客进行一次临时的测试。
